...
{ 
  ...
  uint8_t move_speed = 0, role = 2 ,point_significanse;
  
  double _alpha = 0;
  unsigned select_arrow_y = 0, menu_level = 0, main_menu_arrow_y = 0;
  
  uint16_t change_speed_time = 50, start_trajectory_length = 0;
  
  float cur_dribler_speed = 0;
...
//  }
  ...
  void set_dribler_speed(uint16_t _speed, bool fast_stop = true)
  {
  ...
  bool moveToPoint(point _point, int16_t _speed, int16_t _angle = -255, int16_t _max_speed = 70, int16_t _min_speed = 10)
  {
    ...
    else if(_speed == -1)
    {
      if(use_trajectory)
      {
        ...
        {
          move_speed = constrain(_max_speed, _min_speed, (point_distance) * 2);
        }
      }
      else
        move_speed = constrain(_max_speed, _min_speed, point_distance * 2);
