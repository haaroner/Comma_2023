...
{ 
  ...
  uint16_t change_speed_time = 10, start_trajectory_length = 0;
  
  float cur_dribler_speed = STOP_DRIBLER_SPEED;
...
//    dribler_speed = STOP_DRIBLER_SPEED + constrain(MAX_DRIBLER_SPEED, 0, int(_speed));
  ...
  bool moveToPoint(point _point, int16_t _speed, int16_t _angle = -255, int16_t _max_speed = 38, int16_t _min_speed = 30)
  {
    ...
    if((point_distance > 8 && _point.significanse == 2) ||
       (point_distance > 8 && _point.significanse == 1) ||
