...
//        }
        ...
        if((my_abs(robot_x) <= 20 && forward_distance < 46) ||
            (my_abs(robot_x) > 20 && forward_distance < 48))
          {
...
//          else
//          {
            if(my_abs(ball_angle) < 7 && ((ball_distance > 2 && my_abs(backward_angle) > 155) || (ball_distance > 3 && my_abs(backward_angle) > 135)))/* || (ball_sen_dma.dataReturn(0) > BALL_DETECTION_LIGHTNESS)*/
            {
              if(!kick_en)
                kick_tim = time;
              kick_en = true;
...
//          
        ...
        else if(defender_state == 1)
        {
          ...
          if(my_abs(robot_x) > 35 || time - kick_tim > 2200  || robot_y > 130 || 
           my_abs(ball_angle) > 40 || backward_distance - R0 < -17 || ball_distance <= 1 || 
          ((ball_distance > 2 && my_abs(backward_angle) <= 155) || (ball_distance > 3 && my_abs(backward_angle) <= 135)))
          {
            defender_state = 0;
            kick_over_tim = time;
          }        
